...
Duplicate records can be common when webscraping (the same table may exist on mulitple pages, for example). Removing duplicate records is important because it can lead to incorrect conclusions about observations being higher frequency than they actually are.

For some data, another instance of the same information is important and part of the structure of the set, The stuff that gets deduplicated should be unintentional repetitions that were COLLECTED or GATHERED too frequently for how often they actually occurred.
